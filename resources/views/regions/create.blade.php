<x-app-layout>
    <x-slot name="header">
        <h2 class="outfit text-3xl font-black text-[#3e342f]">Tambah Daerah</h2>
    </x-slot>

    <div class="py-10">
        <div class="max-w-3xl mx-auto">
            <div class="bg-white shadow-[0_4px_20px_rgba(62,52,47,0.03)] border border-[rgba(183,92,28,0.1)] rounded-[2.5rem] p-8 md:p-12 ">
                <form action="{{ route('regions.store') }}" method="POST" class="space-y-6">
                    @csrf
                    <div>
                        <label class="block text-xs font-black text-stone-500 uppercase tracking-widest mb-2">Nama Daerah</label>
                        <input type="text" name="name" required class="w-full bg-white shadow-[0_4px_20px_rgba(62,52,47,0.03)] border border-[rgba(183,92,28,0.1)] rounded-xl px-4 py-3 text-[#3e342f] focus:outline-none focus:border-[#b75c1c] focus:ring-1 focus:ring-[#b75c1c] transition-all" placeholder="Contoh: Jakarta Selatan">
                    </div>
